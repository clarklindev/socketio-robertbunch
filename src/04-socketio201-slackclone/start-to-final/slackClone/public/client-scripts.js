...
// Ensure this script runs after the config script has been loaded
if(!window?.env?.SOCKET_SERVER_URL){
  throw("Socket server URL is not defined");
}

const socketUrl = window.env.SOCKET_SERVER_URL;
// always join the main namespace, thats where the client gets the namespaces from
const socket = io(socketUrl, clientOptions); //available because slack.html <script src="/socket.io/socket.io.js"></script>
